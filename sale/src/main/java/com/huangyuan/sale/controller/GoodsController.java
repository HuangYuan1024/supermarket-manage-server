...
public class GoodsController {
    ...
    public String getByNumber(String number) {
        ...
        if (goods != null) {
            ...
            result.put("code", 400);
            result.put("message", "商品不存在:" + number);
        }
        try {
            return json.writeValueAsString(result);
        } catch (JsonProcessingException e) {
    ...
    public String getByPage(int pageNum, int pageSize, String input) {
        result.clear();
        QueryWrapper<Goods> queryWrapper = new QueryWrapper<>();
        queryWrapper.like(input != null, "c_name", input)
                .or()
                .like(input != null, "c_number", input);
